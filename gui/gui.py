...
Main window was generated by calling:
    pyuic6 bbss_tabbed_gui.ui > main.py
...
class StudentTableFilterProxyModel(QtCore.QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, sourceRow, sourceParent):
        ...
        return (self.filterRegularExpression().match(self.sourceModel().data(index0)).capturedStart() >= 0
                or self.filterRegularExpression().match(self.sourceModel().data(index1)).capturedStart() >= 0
                or self.filterRegularExpression().match(self.sourceModel().data(index2)).capturedStart() >= 0)
...
class StudentTableModel(QtCore.QAbstractTableModel):
    ...
    def student_data(self, index, role=QtCore.Qt.ItemDataRole.DisplayRole):
        student = self.student_list[index.row()]
        return student
...
class BbssGui(QtWidgets.QMainWindow, Ui_BBSS_Main_Window):
    ...
    def __init__(self, parent=None):
        ...
        self.search_students_tableView.setContextMenuPolicy(QtCore.Qt.ContextMenuPolicy.CustomContextMenu)
        self.search_students_tableView.customContextMenuRequested.connect(self.show_context_menu)

    def show_context_menu(self, pos):
        menu = QtWidgets.QMenu(self)
        copy_action = QtGui.QAction(QtGui.QIcon(), 'Kopieren', self)
        #copy_action.triggered.connect(self.testfkt)
        menu.addAction(copy_action)
        history_action = QtGui.QAction(QtGui.QIcon(), 'Historie', self)
        menu.addAction(history_action)
        export_action = QtGui.QAction(QtGui.QIcon(), 'Exportieren...', self)
        menu.addAction(export_action)
        upload_action = QtGui.QAction(QtGui.QIcon(), 'Upload zum Schul-Server...', self)
        ...
        action = menu.exec(global_coordinates)
        if action == copy_action:
            ...
            if 0 <= index.row() < model.rowCount():
                ...
                QtWidgets.QMessageBox.information(self, 'Klassenhistorie',message, QtWidgets.QMessageBox.StandardButton.Ok)
    ...
    def on_import_data(self):
        ...
        QtWidgets.QMessageBox.information(self, 'Schülerdaten importiert.',
                                          message, QtWidgets.QMessageBox.StandardButton.Ok)
    ...
    def on_delete_database(self):
        ...
        reply = QtWidgets.QMessageBox.question(self, 'Datenbank löschen?',
                                               message, QtWidgets.QMessageBox.StandardButton.Yes,
    ...
    def on_import_filter(self, filter_string):
        if filter_string:
            ...
            syntax = QtCore.QRegularExpression.PatternSyntax(QtCore.QRegularExpression.Wildcard)
            case_sensitivity = QtCore.Qt.CaseInsensitive
            regExp = QtCore.QRegularExpression(filter_string, case_sensitivity, syntax)
